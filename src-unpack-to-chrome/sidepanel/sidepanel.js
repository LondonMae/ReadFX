...
const palettes = {
  default: [
    ...
    "c59ca8",
    "#3b3e4d",
    "linear-gradient(144deg, #d3bbf2, #87c0e3, #bbe8e1)",
    "linear-gradient(144deg, #d3bbf2, #87c0e3, #bbe8e1)",
  ],
  frutiger: [
    "cde7b0",
...
function toggleTheme() {
  ...
  document.getElementById("current-theme").innerText =
    Object.keys(palettes)[current_theme];
}
...
function deletehighlight(hash, index) {
  ...
  chrome.storage.local.get("highlights").then((items) => {
...
function update_colors(e) {
  ...
  chrome.storage.local.get("colors").then((items) => {
...
function openPdf() {
  chrome.tabs.query({ active: true, lastFocusedWindow: true }, (tabs) => {
    ...
    let newtab = chrome.tabs.create({
      url: "pdf_parser/index.html?data=" + encodedString,
    });
  });
  // chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
  //     // make sure the status is 'complete' and it's the right tab
...
function cleanPage() {
  ...
  document.head.innerHTML =
    ...
    chrome.runtime.getURL("/reading_mode/readingmode.css") +
    `">`;
}
...
document
  .getElementById("list-highlights-button")
...
document
  .getElementById("save-notebook-button")
  .addEventListener("click", () => {
    saveNotebook();
    open_notes();
  });
...
fontsizeslider.addEventListener("change", (e) => {
  document.getElementById("fontsizeindicator").innerText =
